...
function getValue(){
    drawText = document.field.q.value
    return drawText ;
}
...
function wrapText(ctx, text, x, y, mW, cW, lH, clr, hTagclr) {
    ...
    for (var n= 0; n<words.length; n++) {
        ...
        if(r == true) { //change color of the rest of sentense if true
            ...
            string = string.replace('_', ''); //remove the underscore
            w = ctx.measureText(string).width; //recalculate width to remove whitespaces left
        }else{
...
function getImage(canvas) {
    ...
    $('.createImagebtn .label').html("Just a sec...")
...
function loadTweets() {
    ...
    $('.writeform')
        ...
        .append('<h2 style="border-bottom: 1px solid #bbb; padding: 3px; margin: 0">tweets ' +
        '<span id="showRefreshing" style="font-size: 11px; color: #87cefa;">loading</span></h2>' +
        '<div class="Tw"></div>')

    $('.Tw') //show loading image
        .animate({'height': 300, 'width': '100%', 'overflowY': 'scroll',
            'overflowX': 'hidden', 'border-bottom': '1px solid #aaa'}, 600, function() {
            $(this).append('<img id="loadingCircle" src="./images/325.gif" style="position: relative; top: 40%; left: 40%" alt="Loading Tweets..." />')
        })

    $(pFclone).insertAfter('.Tw')
            ...
            $('.tweets').bind('click', function() {
                ...
                $.map(tweets, function(a,b) {
                    if(twId == a.id) {
                        ...
                        drawCanvas(a.tweet)
                        setValue(a.tweet) //load into text area
...
(function ($) {
    ...
    $.fn.extend({
        ...
        loadUtilities: function() {
            ...
            $('body').append('<div id="loadingPref" style="position: absolute; top:0; left: 0; width:100%; height: 100%; background: #fefefe; opacity: 0.8"><h2 id="loadingPrefh2"  style="text-align: center; margin-top: 30%">Loading Fonts...</h2></div>');
            ...
            WebFontConfig = {
                google: {
                    families:[
                        ...
                        "Macondo", "McLaren", "Metamorphous", "MedievalSharp", "Miniver",
                ...
                active: function() { //when finished
                    ...
                    console.log("All the Fonts have been loaded...")
                    ...
                    $.getJSON('./js/utilities.json', function(json) {
                        ...
                        //console.log(fonts)
                        font = fonts[12].string //for testing purposes
